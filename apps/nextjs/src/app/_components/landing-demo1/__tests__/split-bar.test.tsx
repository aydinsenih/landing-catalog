...
function renderBar() {
  return render(<SplitBar label={DEMO1_SPLIT_LABEL} rows={DEMO1_SPLIT_ROWS} />);
}
...
describe("SplitBar", () => {
  it("names the split it shows", () => {
    ...
    expect(screen.getByText(DEMO1_SPLIT_LABEL)).toBeInTheDocument();
  });

  it("gives every collaborator a share, a name, a role, and a duty", () => {
    renderBar();

    for (const row of DEMO1_SPLIT_ROWS) {
  ...
  it("weights each collaborator column by the share it reads", () => {
    ...
    expect(columns.map((column) => column.style.flexGrow)).toStrictEqual(
      DEMO1_SPLIT_ROWS.map((row) => String(row.share)),
    );
  });
