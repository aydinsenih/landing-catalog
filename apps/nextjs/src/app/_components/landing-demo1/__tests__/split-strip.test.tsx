...
import { DEMO1_SPLIT_ROWS } from "@acme/constants";
...
describe("SplitStrip", () => {
  it("draws one segment for each share", () => {
    const { container } = render(<SplitStrip rows={DEMO1_SPLIT_ROWS} />);

    expect(segmentsOf(container)).toHaveLength(DEMO1_SPLIT_ROWS.length);
  });
  ...
  it("weights each segment by the share it stands for", () => {
    ...
    ).toStrictEqual(DEMO1_SPLIT_ROWS.map((row) => String(row.share)));
  });
  ...
  it("gives every share its own colour", () => {
    ...
    expect(new Set(tones).size).toBe(DEMO1_SPLIT_ROWS.length);
  });

  it("takes the shape it is asked for, and holds its own without one", () => {
    const { container: sized } = render(
      <SplitStrip rows={DEMO1_SPLIT_ROWS} className="h-1.5" />,
    );
    expect(sized.querySelector("[data-slot='split-strip']")).toHaveClass(
      "h-1.5",
    );

    const { container: bare } = render(<SplitStrip rows={DEMO1_SPLIT_ROWS} />);
